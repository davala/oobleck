...
function buildUser(category) {
  switch (category) {
    ...
    case 'process':
      return `Generate a single process-based creative prompt about workflow, decision-making, or creative constraints. Focus on how the user thinks and works — not physical stunts or literal body movements. Do not name any artist, producer, or collaborator. The prompt should be a single oblique idea — not a multi-step instruction. Be lateral and surprising. Never state the obvious. Return only the prompt itself, no setup, no attribution, no explanation. Do not begin the prompt with the words 'Work', 'Start', 'Begin', or 'Try'. One or two sentences maximum.`;
    case 'wildcard':
